...
function normalizeImages(input) {
  ...
  return list.map((item, index) => ({ image_url: text(typeof item === "string" ? item : item.image_url, 2000), sort_order: Number.isInteger(Number(typeof item === "string" ? index : item.sort_order)) ? Number(typeof item === "string" ? index : item.sort_order) : index, is_featured: Boolean(typeof item === "string" ? index === 0 : item.is_featured) })).filter(item => item.image_url);
}
function normalizeVariants(input) {
  const list = Array.isArray(input) ? input : [];
  return list.map(item => ({ color: text(item.color, 80), size: text(item.size, 40), stock: Number(item.stock) })).filter(item => (item.color || item.size) && Number.isInteger(item.stock) && item.stock >= 0).slice(0, 200);
}
function normalizeProduct(input, { requireId = false } = {}) {
  const name = text(input.name, 120), id = slug(input.id || name), price = Number(input.price), stock = Number(input.stock), sortOrder = Number(input.sort_order);
